...
with beam.Pipeline() as p:
  ...
  wordsStartingWithA: beam.PCollection[str] = \
      p | 'Words starting with A' >> beam.Create(['apple', 'ant', 'arrow'])

  wordsStartingWithB: beam.PCollection[str] = \
      p | 'Words starting with B' >> beam.Create(['ball', 'book', 'bow'])

  results: beam.PCollection[str] = [wordsStartingWithA, wordsStartingWithB] | beam.Flatten()
  (results | LogElements())
