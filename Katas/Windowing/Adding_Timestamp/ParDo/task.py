...
class Event:
    def __init__(self, id: str, event: str, timestamp: datetime.datetime):
        self.id = id
        self.event = event
...
class AddTimestampDoFn(beam.DoFn):
    ...
    def process(self, element: Event, *args, **kwargs) -> Iterable:
        yield window.TimestampedValue(element, element.timestamp.timestamp())


with beam.Pipeline() as p:
    p: beam.Pipeline

    events: beam.PCollection[Event] = p | beam.Create([
    ...
    with_timestamp: beam.PCollection[Event] = events | beam.ParDo(AddTimestampDoFn())
    (with_timestamp | LogElements(with_timestamp=True))